...
# === Custom theme styles (truncated for brevity) ===
# Custom theme styles
st.markdown("""
<style>
...
if uploaded_file:
    ...
    if model_choice == "DenseNet121 (Keras)":
    ...
    elif model_choice == "EfficientNetB0 (Keras) with LIME":
        ...
        def predict_fn(images):
            return model.predict(np.array([img / 255.0 for img in images]))
        ...
        overlay_img = mark_boundaries(temp, mask)

        prediction = model.predict(preprocess(img_np))
        pred_class = "TB Detected" if prediction[0][0] > 0.5 else "Normal"
        pred_prob = prediction[0][0] if prediction[0][0] > 0.5 else 1 - prediction[0][0]

    else:
        model_type = "resnet" if "ResNet" in model_choice else "vgg"
    ...
    if st.button("Download Explanation Image"):
        ...
        st.download_button(
            ...
            file_name="lime_explanation.jpg",
            mime="image/jpeg"
        )
else:
