...
def make_gradcam_pytorch(model, img_tensor, final_conv, model_type,class_index):
    features = []
    gradients = []
...
if uploaded_file:
    ...
    if model_choice == "DenseNet121":
        ...
        # Show label based on predicted class
        #pred_class = "TB Detected" if pred_index == 1 else "Normal"
        #pred_prob = prob
        class_labels = {0: "TB Detected", 1: "Normal"}  # Flip if TB was label 0
        pred_class = class_labels[pred_index]
      
        
        # Grad-CAM overlay
        heatmap, prob = make_gradcam_pytorch(model, img_tensor, final_conv, model_type,pred_index)
        overlay_img = overlay_heatmap(heatmap, img)
    ...
    #st.success("Predicted class index:", pred_index)
    #st.success("Probabilities:", probs.detach().numpy())
